    ...
    public void onMove(PlayerMoveEvent event) {
        ...
        if(player.isGliding()) {
            ...
            if (100 < player.getLocation().getChunk().getInhabitedTime()){
                ...
                player.spigot().sendMessage(ChatMessageType.ACTION_BAR, TextComponent.fromLegacyText("Lassabban! Újonnan generált részhez érkeztél!"));
            ...
            // AntiCheat Section
            if (3F < data.get(player + "deltaXZ") && data.get(player + "deltaXZ") == ((double) Math.sqrt(Math.pow(event.getTo().getX() - event.getFrom().getX(), 2) + Math.pow(event.getTo().getZ() - event.getFrom().getZ(), 2)))) {
            ...
            } else if(data.get(player + "deltaXZ") < 3F){
